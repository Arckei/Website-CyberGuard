...
// ---------- Class lesson feed (announcements + optional file attachment) ----------

export const ALLOWED_LESSON_TYPES = new Set(["pdf", "docx", "ppt", "pptx"]);

export function lessonFileType(fileName = "") {
  return fileName.split(".").pop()?.toLowerCase() || "file";
}

export function isAllowedLessonFile(file) {
  return ALLOWED_LESSON_TYPES.has(lessonFileType(file.name));
}
...
// Returns every post for a class, newest first. Old classes may still have
// data under the legacy `lessons` array from before file uploads actually
// went anywhere real — those are folded in here (as read-only, unopenable
// attachments) so nothing a teacher already posted just disappears once
// this feed ships.
export function getClassPosts(klass) {
  if (!klass) return [];
  const posts = Array.isArray(klass.posts) ? klass.posts : [];
  const legacyLessons = Array.isArray(klass.lessons) ? klass.lessons : [];
  const postIds = new Set(posts.map((post) => post.id));

  const migrated = legacyLessons
    .filter((lesson) => lesson?.id && !postIds.has(lesson.id))
    .map((lesson) => ({
      id: lesson.id,
      authorName: klass.teacher || "Teacher",
      message: "",
      attachment: {
        name: lesson.name || "Lesson file",
        extension: (lesson.type || lessonFileType(lesson.name || "")).toLowerCase(),
        size: lesson.size || 0,
        downloadUrl: null
      },
      createdAt: lesson.uploadedAt || new Date(0).toISOString(),
      legacy: true
    }));

  return [...posts, ...migrated].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
}

// PDFs render fine directly in a browser tab. Office formats (ppt/pptx/doc/docx)
// don't, so those go through Microsoft's public Office viewer, which can embed
// any file reachable at a public URL (like a raw.githubusercontent.com link).
export function attachmentViewerUrl(attachment) {
  if (!attachment?.downloadUrl) return null;
  const ext = (attachment.extension || "").toLowerCase();
  if (ext === "pdf") return attachment.downloadUrl;
  return `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(attachment.downloadUrl)}`;
}

// Shared markup for one feed entry — used by both the admin compose page
// and the student-facing lessons feed, so the two always look and behave
// the same way.
export function renderPostMarkup(post) {
  const hasAttachment = Boolean(post.attachment);
  const viewerUrl = hasAttachment ? attachmentViewerUrl(post.attachment) : null;

  const attachmentMarkup = hasAttachment ? `
    <div class="post-attachment">
      <span class="lesson-type">${escapeHtml((post.attachment.extension || "file").toUpperCase())}</span>
      <div class="post-attachment-info">
        <strong>${escapeHtml(post.attachment.name || "Attachment")}</strong>
        <p class="muted">${formatFileSize(post.attachment.size)}</p>
      </div>
      <div class="post-attachment-actions">
        ${viewerUrl ? `<a class="btn ghost" href="${escapeHtml(viewerUrl)}" target="_blank" rel="noopener">View</a>` : ""}
        ${post.attachment.downloadUrl ? `<a class="btn ghost" href="${escapeHtml(post.attachment.downloadUrl)}" target="_blank" rel="noopener" download>Download</a>` : `<span class="muted">Preview unavailable (uploaded before file storage)</span>`}
      </div>
    </div>
  ` : "";

  return `
    <article class="post-card${post.legacy ? " post-legacy" : ""}">
      <header class="post-card-header">
        <strong>${escapeHtml(post.authorName || "Teacher")}</strong>
        <span class="muted">${escapeHtml(formatTimestamp(post.createdAt))}</span>
      </header>
      ${post.message ? `<p class="post-message">${escapeHtml(post.message)}</p>` : ""}
      ${attachmentMarkup}
    </article>
  `;
}

export function renderPostFeed(container, posts) {
  if (!container) return;
  container.innerHTML = posts.length
    ? posts.map(renderPostMarkup).join("")
    : `
      <div class="empty-lessons">
        <strong>No announcements yet.</strong>
        <p class="muted">Nothing has been posted to this class yet.</p>
      </div>
    `;
}

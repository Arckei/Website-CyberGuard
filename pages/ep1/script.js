...
// checklist. Both episodes still add their points to the SAME per-class
// score (klass.scores[uid]), so "pts" on this page and on Episode 0 add up
// on the leaderboard.
...
// services/game-score.js). Checks off every Episode 1 task in one go via
// CyberGuardBridge.completeEpisode1, and bails out once it's already fully
// checked off so a duplicate event doesn't re-award points.
function handleEpisodeOneComplete(score) {
  const state = getState();
  const tasks = getEpisodeProgress(state);
  const alreadyComplete = EPISODE_ONE_TASKS.every((task) => tasks[task.id]);
